...
# Defina o comando docker compose apenas se estiver disponível
...
.PHONY: docker/build
docker/build:
	docker build -t payment-gateway:latest -f .setup/build/Dockerfile --build-arg GITLAB_USER=${GITLAB_USER} --build-arg GITLAB_PASSWORD=${GITLAB_PASSWORD} .
...
.PHONY: docker/logs
docker/logs:
	docker logs --tail 1000 -f payment-gateway
